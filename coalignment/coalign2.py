...
for i in range(len(L3_path)):
    if USE_DATEAVG_METHOD:
        ...
        month = dateavg.astype(object).month
        ...
        FSI_path.append(list(FSI_folder.glob(f"{year}/{month}/{day}/*fsi174-image_{year}{month}{day}T{hour}*"))[0])
        path_fsi = Path(str(FSI_path[i]))
        path_hri = Path(str(L3_path[i]))
    ...
    lag_crval1 = np.arange(-60, 60, 2)
    lag_crval2 = np.arange(-60, 60, 2)
    lag_crota = [0]
    lag_cdelta1 = [0]
    lag_cdelta2 = [0]
